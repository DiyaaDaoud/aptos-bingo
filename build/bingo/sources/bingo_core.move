module overmind::bingo_core {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let (rsrc_sgnr, rsrc_sgnr_cap) = account::create_resource_account(admin, b"BINGO");
        // TODO: Register the resource account with AptosCoin
        coin::register<AptosCoin>(&rsrc_sgnr);
        // TODO: Move State resource to the admin's address
        move_to<State>(admin, State {
            bingo : signer::address_of(&rsrc_sgnr)
        });
        // TODO: Move Bingo resource to the resource account's address
        move_to<Bingo>(&rsrc_sgnr, Bingo {
            games: simple_map::create(),
            cap: rsrc_sgnr_cap,
            create_game_events: account::new_event_handle<CreateGameEvent>(&rsrc_sgnr),
            cancel_game_events: account::new_event_handle<CancelGameEvent>(&rsrc_sgnr)
        });
    }
    ...
    */
    public entry fun insert_number(admin: &signer, game_name: String, number: u8) acquires State, Bingo {
        ...
        let current_game = simple_map::borrow_mut(&mut bingo.games, &game_name);
        assert_game_already_stared(current_game.start_timestamp);
        // TODO: Assert that the drawn number is not a duplicate
        assert_number_not_duplicated(&current_game.drawn_numbers, &number);
        // TODO: Add the drawn number to game's drawn numbers
        vector::push_back(&mut current_game.drawn_numbers,number);
        ...
        event::emit_event<InsertNumberEvent>(&mut current_game.insert_number_events, insertNumberEvent);
    }
    ...
    */
    public entry fun join_game(player: &signer, game_name: String, numbers: vector<vector<u8>>) acquires State, Bingo {
        ...
        let current_game = simple_map::borrow_mut(&mut bingo.games, &game_name);
        assert_game_not_started(current_game.start_timestamp);
        // TODO: Assert that the player has enough APT to join the game
        assert_suffiecient_funds_to_join(signer::address_of(player), current_game.entry_fee);
        // TODO: Assert that the player has not joined the game yet
        assert_player_not_joined_yet(&current_game.players, &signer::address_of(player));
        // TODO: Add the player to the game's list of players
        simple_map::add(&mut current_game.players, signer::address_of(player), numbers);
        // TODO: Transfer entry fee from the player to the bingo PDA
        coin::transfer<AptosCoin>(player, state.bingo, current_game.entry_fee);
        ...
        event::emit_event<JoinGameEvent>(&mut current_game.join_game_events, joinGameEvent);
    }
    ...
    */
    public entry fun bingo(player: &signer, game_name: String) acquires State, Bingo {
        ...
        let current_game = simple_map::borrow_mut(&mut bingo.games, &game_name);
        assert_game_not_finished(current_game.is_finished);
        // TODO: Assert that the player joined the game
        assert_player_joined(&current_game.players, &signer::address_of(player));
        // TODO: Assert that the player has bingo
        let player_nums = simple_map::borrow(&current_game.players, &signer::address_of(player));
        assert_player_has_bingo(&current_game.drawn_numbers, *player_nums);
        ...
        let rsrc_acct = account::create_signer_with_capability(&bingo.cap);
        let amount = current_game.entry_fee * simple_map::length(&current_game.players);
        coin::transfer<AptosCoin>(&rsrc_acct, signer::address_of(player), amount);
        ...
        event::emit_event<BingoEvent>(&mut current_game.bingo_events, bingoEvent);
    }
    ...
    */
    public entry fun cancel_game(admin: &signer, game_name: String) acquires State, Bingo {
        ...
        let total_num_of_players = simple_map::length(&current_game.players);
        let (players_addresses, _) = simple_map::to_vec_pair(current_game.players);
        let rsrc_acct = account::create_signer_with_capability(&bingo.cap);
        let counter = 0;     
        while (counter < total_num_of_players) {
            let player_address = vector::borrow(&players_addresses, counter);
            coin::transfer<AptosCoin>(&rsrc_acct, *player_address, current_game.entry_fee);
            counter = counter + 1;
        };
    ...
    */
    fun check_player_numbers(drawn_numbers: &vector<u8>, player_numbers: vector<vector<u8>>): bool {
        ...
        let final_res = vector::empty<vector<Option<u8>>>();
        let total_num_of_drawn = vector::length(drawn_numbers);
        let col_counter = 0;
        while (col_counter < 5) {
            let checked_row = vector::empty<Option<u8>>();
            let column_player_numbers = vector::borrow_mut(&mut player_numbers, col_counter);
            let row_counter = 0;
            while (row_counter < 5) {
                let current_number = vector::borrow_mut(column_player_numbers ,row_counter);
                let drawn_nums_counter = 0;
                let is_match = false;
                while (drawn_nums_counter < total_num_of_drawn) {
                    let drawn_number = vector::borrow(drawn_numbers,drawn_nums_counter);
                    if(*current_number == *drawn_number) {
                        is_match = true;
                        break
                    };
                    drawn_nums_counter = drawn_nums_counter + 1;
                };
        ...
        //      2) If a number is 0, then replace it with Option::None
                if(is_match == true || *current_number == 0) {
                    vector::push_back(&mut checked_row, option::none());
        //      3) If a number does not match any number in the drawn numbers, then replace it with Option::Some
                } else {
                    vector::push_back(&mut checked_row, option::some(1));
                };
                row_counter = row_counter + 1;
            };
            vector::push_back(&mut final_res, checked_row);
            col_counter = col_counter + 1;
        };
        // TODO: Call check_columns, check_diagonals and check_rows and return true if any of those returns true
        let check_columns_res = check_columns(&mut final_res);
        let check_diagonals_res = check_diagonals(&mut final_res);
        let check_rows_res = check_rows(&mut final_res);
        let returned_value = false;
        if(check_columns_res || check_diagonals_res || check_rows_res) {
            returned_value = true;
        };
        returned_value
    }
    ...
    */
    inline fun check_columns(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        let final_res = false;
        while (col_counter < 5) {
            let column_player_numbers = vector::borrow(player_numbers,col_counter);
            let row_counter = 0;
            let col_check = true;
            while (row_counter < 5) {
                let current_number = vector::borrow(column_player_numbers,row_counter);
                if (*current_number != option::none()) {
                    col_check = false;
                    break
                };
                row_counter = row_counter + 1;
    ...
    */
    inline fun check_rows(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        while (row_counter < 5) {
            ...
            let row_check = true;
            while (col_counter < 5) {
                let col_player_numbers = vector::borrow(player_numbers,col_counter);
                let current_number = vector::borrow(col_player_numbers,row_counter);
                if (*current_number != option::none()) {
                    row_check = false;
                    break
                };
    ...
    */
    inline fun check_diagonals(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        let final_res = false;    
        let col_counter = 0;
        let main_diagonal_check = true;
        let secondary_diagonal_check = true;
        while (col_counter < 5) {
            let main_diagonal_col = vector::borrow(player_numbers, col_counter);
            let current_main_number = vector::borrow(main_diagonal_col, col_counter);
            if (*current_main_number != option::none()) {
                main_diagonal_check = false;
            };
            
            let secondary_diagonal_col = vector::borrow(player_numbers, 4 - col_counter);
            let current_secondary_number = vector::borrow(secondary_diagonal_col, col_counter);
            if (*current_secondary_number != option::none()) {
                secondary_diagonal_check = false;
            };
            col_counter = col_counter + 1;
        };
        if (main_diagonal_check == true || secondary_diagonal_check == true) {
            final_res = true;
        };    
        final_res
    }
    ...
    inline fun assert_correct_amount_of_picked_numbers(picked_numbers: &vector<vector<u8>>) {
        ...
        let number_of_columns = vector::length(picked_numbers);
        assert!(number_of_columns==5, INVALID_AMOUNT_OF_COLUMNS_IN_PICKED_NUMBERS);
        let col_counter = 0;
        while (col_counter < 5) {
            let current_column = vector::borrow(picked_numbers,col_counter);
            let current_column_length = vector::length(current_column);
            assert!(current_column_length == 5, INVALID_AMOUNT_OF_NUMBERS_IN_COLUMN);
            col_counter = col_counter + 1;
        }
    }
    ...
    inline fun assert_numbers_are_picked_correctly(picked_numbers: &vector<vector<u8>>) {
        ...
        while (col_counter < 5) {
            ...
            let row_counter = 0;
            while (row_counter < 5) {
                let current_number = vector::borrow(current_column,row_counter);
                if (col_counter == 2 && row_counter==2) {
                    assert!(*current_number == 0, COLUMN_HAS_INVALID_NUMBER);
                } else {
                    assert!(*current_number >= ((15*(col_counter)+1) as u8) && *current_number <= ((15*(col_counter+1)) as u8),COLUMN_HAS_INVALID_NUMBER);
                };
                row_counter = row_counter +1;
            };
            col_counter = col_counter +1;
    ...
    inline fun assert_suffiecient_funds_to_join(player: address, entry_fee: u64) {
        ...
        let players_funds = coin::balance<AptosCoin>(player);
        assert!(players_funds >= entry_fee, INSUFFICIENT_FUNDS)
    }
